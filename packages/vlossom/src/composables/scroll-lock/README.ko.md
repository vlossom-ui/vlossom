...
- 공유 store를 통해 해석된 element 단위로 잠금을 참조 카운팅하므로, 같은 element를 여러 오버레이가 잠그면 마지막 소유자가 해제할 때까지 잠금이 유지됩니다
...
- 이미 잠긴 상태에서 `lock`을 호출하거나, 이미 잠금 해제된 상태에서 `unlock`을 호출하면 아무 동작도 하지 않습니다(`isLocked`가 소유자별로 가드합니다).
- 잠금은 해석된 element 단위로 모든 호출자 간에 공유·참조 카운팅됩니다. 원래 상태는 첫 잠금 시점에 캡처되고 마지막 소유자가 해제할 때만 복원되므로, 닫는 순서와 무관합니다.
