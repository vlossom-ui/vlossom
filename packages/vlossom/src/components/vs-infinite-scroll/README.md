...
```html
<template>
    <vs-infinite-scroll :initial-index="10">
...
| `initialIndex` | `string \| number` | -       | -        | 초기 스크롤 위치로 이동할 자식 요소의 인덱스                               |
...
| --------------- | --------- | --------------------------------------- |
| `scrollToIndex` | `number`  | 지정한 인덱스의 자식 요소로 스크롤 이동 |
...
- **초기 스크롤 위치**: `initialIndex` prop으로 마운트 시 특정 위치로 자동 스크롤
