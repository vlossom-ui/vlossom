<template>
    <div ref="triggerRef" class="vs-select-trigger" @click="$emit('click')">
        <div class="vs-select-value" style="flex: 1">
            ...
            <template v-else-if="multiple">
                <template v-if="collapseChips && selectedOptions.length > 1">
                    <vs-chip small :closable="closableChips" @close="$emit('deselect', selectedOptions[0].id)">
...
export default defineComponent({
    ...
    setup(props) {
        const { isEmpty, selectedOptions } = toRefs(props);
