...
| `styleSet`      | `string \| VsTableStyleSet`                    |          | 컴포넌트 스타일 세트                                                                                               |
| `columns`       | `VsTableColumnDef[] \| string[]`               | `[]`     | 컬럼 정의. 비어 있으면 첫 아이템의 키를 사용합니다                                                                 |
| `items`         | `VsTableItem[]`                                | `[]`     | 데이터 행. 테이블이 변경하지 않습니다                                                                              |
...
| `loading`       | `boolean`                                      | `false`  | 로딩 상태 표시 및 검색/페이지네이션/드래그/선택 비활성화                                                           |
| `page`          | `number`                                       |          | 현재 페이지 인덱스 (0부터 시작), v-model                                                                           |
| `pageSize`      | `number`                                       |          | 페이지당 행 수, v-model. 기본값은 pageSizeOptions의 첫 번째 값                                                     |
| `pagedItems`    | `VsTableItem[]`                                | `[]`     | 현재 페이지의 행, v-model (출력 전용)                                                                              |
...
| `totalItems`    | `VsTableItem[]`                                | `[]`     | 검색과 정렬을 거친 전체 행(화면 순서), v-model (출력 전용)                                                         |
...
| ---------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| `click-cell`           | `(cell: VsTableCell, event: MouseEvent)`                                   | 바디 셀을 클릭했을 때                                                            |
| `click-row`            | `(item: VsTableItem, index: number, event: MouseEvent)`                    | 바디 행을 클릭했을 때                                                            |
| `select-row`           | `(item: VsTableItem, index: number, selected: boolean, event: MouseEvent)` | 행이 선택되거나 해제됐을 때                                                      |
| `expand-row`           | `(item: VsTableItem, index: number, expanded: boolean, event: MouseEvent)` | 행이 펼쳐지거나 접혔을 때                                                        |
| `drag`                 | `SortableEvent`                                                            | 드래그 앤 드롭으로 순서가 바뀐 뒤                                                |
| `search`               | `(items: VsTableItem[], searchText: string)`                               | 검색 시. 필터링 후 화면에 보이는 행과 함께 발생합니다                            |
| `paginate`             | `(nextPage: number, pageSize: number)`                                     | 페이지 또는 페이지 크기가 바뀌었을 때 (페이지 크기가 바뀌면 페이지는 0으로 리셋) |
| `update:selectedItems` | `VsTableItem[]`                                                            | 선택된 행이 바뀌었을 때                                                          |
| `update:page`          | `number`                                                                   | 현재 페이지가 바뀌었을 때                                                        |
| `update:pageSize`      | `number`                                                                   | 페이지 크기가 바뀌었을 때                                                        |
| `update:pagedItems`    | `VsTableItem[]`                                                            | 현재 페이지의 행이 바뀌었을 때                                                   |
| `update:totalItems`    | `VsTableItem[]`                                                            | 검색/정렬을 거친 전체 행이 바뀌었을 때                                           |
...
| `caption`      |                                                     | 테이블 캡션                                                 |
| `header-[key]` | `{ item: VsTableColumnDef, value, colIdx, rowIdx }` | 특정 컬럼 키의 헤더 셀                                      |
| `item-[key]`   | `{ item: VsTableItem, value, colIdx, rowIdx }`      | 특정 컬럼 키의 아이템 셀                                    |
| `select`       | `{ item, value, rowIdx }`                           | 헤더와 바디 행의 선택 컬럼 셀                               |
...
`select` 슬롯은 헤더와 모든 바디 행의 선택 컬럼 셀을 한 번에 교체합니다. 헤더 셀에서는 `item`이 `null`이고 `value`가 전체 선택 상태, `rowIdx`가 `0`이므로 `item`으로 헤더 여부를 구분합니다. 셀 아무 곳이나 클릭하면 선택이 토글되므로 커스텀 콘텐츠에 별도 클릭 핸들러는 필요 없으며, `selectable`에서 제외된 행은 토글되지 않습니다.
