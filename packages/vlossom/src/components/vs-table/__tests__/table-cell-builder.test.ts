...
describe('TableCellBuilder', () => {
    ...
    it('같은 위치에서 새 인스턴스로 교체되면 행 key와 셀 id가 유지된다', () => {
        const columnDefs: VsTableColumnDef[] = [{ key: 'name', label: '이름' }];
        ...
        builder.updateItems([{ name: 'Alice Kim' }, { name: 'Bob Smith' }]);
        const { rows: nextRows } = builder.build();

        expect(nextRows[0].key).toBe(aliceKey);
        expect(nextRows[0].cells[0].id).toBe(aliceCellId);
        expect(nextRows[0].cells[0].value).toBe('Alice Kim');
        expect(nextRows[1].key).toBe(bobKey);
    });
