...
import VsTable2 from './../VsTable2.vue';
import type { VsTable2Cell, VsTable2ColumnDef, VsTable2Item } from './../types';

const defaultColumns = ['name', 'age'];
const labeledColumns: VsTable2ColumnDef[] = [
    { key: 'name', label: '이름' },
    { key: 'age', label: '나이' },
];
const sortableColumns: VsTable2ColumnDef[] = labeledColumns.map((column) => ({ ...column, sortable: true }));
const tableItems = [
...
describe('VsTable2', () => {
    ...
    describe('cell slot 우선순위', () => {
        it('`header-${colKey}` / `item-${colKey}` 슬롯이 기본 렌더링보다 우선한다', async () => {
            const wrapper = mountTable({
                slots: {
                    'header-name': ({ item }: { item: VsTable2ColumnDef }) => `HEADER-${item.key}`,
                    'item-name': ({ item }: { item: VsTable2Item }) => `ITEM-${item.name}`,
                },
            });
        ...
        it('`item-${colKey}` 슬롯이 위치 기반 슬롯보다 우선한다', async () => {
            const wrapper = mountTable({
                slots: {
                    'item-age': ({ item }: { item: VsTable2Item }) => `AGE-${item.age}`,
        ...
        it('"header", "item" 슬롯은 최후 fallback으로 적용된다', async () => {
            const wrapper = mountTable({
                slots: {
                    header: ({ colIdx, item }: { colIdx: number; item: VsTable2ColumnDef }) =>
                        `HEADER-${colIdx}-${item.key}`,
                    item: ({ item }: { item: VsTable2Item }) => `ITEM-${item.id}`,
                },
            });
        ...
        it('camelCase colKey가 literal 형태의 슬롯 이름과 매칭된다', async () => {
            const wrapper = mountTable({
                ...
                slots: {
                    'header-firstName': ({ item }: { item: VsTable2ColumnDef }) => `HEAD-${item.key}`,
                    'item-firstName': ({ item }: { item: VsTable2Item }) => `FIRST-${item.firstName}`,
                },
            });
    ...
    describe('emits', () => {
        it('셀 클릭 시 click-cell을 item 기반 payload로 발생시킨다', async () => {
            ...
            const [cell, event] = lastEmit(wrapper, 'click-cell') as [VsTable2Cell, Event];
        ...
        it('셀 클릭 시 click-row를 item과 index와 함께 발생시킨다', async () => {
            ...
            const [item, index, event] = lastEmit(wrapper, 'click-row') as [VsTable2Item, number, Event];
        ...
        it('selectable이 false를 반환하는 행에서는 select-row가 발생하지 않는다', async () => {
            const wrapper = mountTable({ props: { selectable: (item: VsTable2Item) => item.name === 'Bob' } });
        ...
        it('검색 시 search 이벤트를 (표시 중인 items, 검색어)와 함께 발생시킨다', async () => {
            ...
            const [items, searchText] = lastEmit(wrapper, 'search') as [VsTable2Item[], string];
    ...
    describe('expandable', () => {
        it('expand 슬롯이 있으면 확장 버튼과 패널을 렌더링한다', async () => {
            const wrapper = mountTable({
                slots: {
                    expand: ({ item, rowIdx }: { item: VsTable2Item; rowIdx: number }) =>
        ...
        it('expand(index) / collapse(index) 메서드로 행을 펼치고 접는다', async () => {
            const wrapper = mountTable({
                slots: { expand: ({ item }: { item: VsTable2Item }) => h('div', {}, String(item.name)) },
            });
        ...
        it('없는 index나 expandable하지 않은 행은 expand(index)로 펼쳐지지 않는다', async () => {
            const wrapper = mountTable({
                props: { expandable: (item: VsTable2Item) => item.name === 'Alice' },
                slots: { expand: ({ item }: { item: VsTable2Item }) => h('div', {}, String(item.name)) },
            });
    ...
    describe('size', () => {
        it.each(['xs', 'sm', 'md', 'lg', 'xl'] as const)('size="%s"이면 vs-%s 클래스가 적용된다', async (size) => {
            ...
            expect(wrapper.find('.vs-table2').classes()).toContain(`vs-${size}`);
        });
        ...
        it('기본 size는 md이다', async () => {
            ...
            expect(wrapper.find('.vs-table2').classes()).toContain('vs-md');
        });
    });
});
