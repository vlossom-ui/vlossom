...
const meta: Meta<typeof VsTable2> = {
    title: 'Components/Base Components/VsTable2',
    component: VsTable2,
    parameters: {
        docs: {
            description: {
                component:
                    'VsTable2는 컬럼 정의와 아이템을 기반으로 데이터를 렌더링하며, 슬롯을 통해 헤더/바디를 자유롭게 커스텀할 수 있습니다. ' +
    ...
    render: (args: any) => ({
        ...
        template: '<vs-table2 v-bind="args" />',
    }),
    args: {
        columns: baseColumns,
        items: baseItems,
...
export const CustomSlots: Story = {
    render: () => ({
        ...
        template: `
            <vs-table2 :columns="columns" :items="items">
                <template #caption>
...
export const Selectable: Story = {
    render: () => ({
        components: { VsTable2 },
        setup() {
            const selectedItems = ref<VsTable2Item[]>([]);
            const selectable = (item: VsTable2Item) => item.name !== 'Jim';
...
export const Expandable: Story = {
    render: () => ({
        ...
        setup() {
            ...
            const expandable = (item: VsTable2Item) => Number(item.age) >= 30;
            return { columns: baseColumns, items, expandable };
        },
        template: `
            <vs-table2 :columns="columns" :items="items" :expandable="expandable" item-key="id">
    ...
    render: () => ({
        ...
        template: `
            <div style="height: 20rem; overflow: auto">
                <vs-table2 :columns="columns" :items="items" sticky-header item-key="id" />
            </div>
        `,
    }),
    parameters: {
        ...
        template: `
            <div class="flex flex-col gap-6">
                <div v-for="size in sizes" :key="size">
                    ...
                    <vs-table2 :columns="columns" :items="items" :size="size" />
                </div>
            </div>
        `,
    }),
...
export const ServerMode: Story = {
    render: () => ({
        ...
        setup() {
            ...
            const items = ref<VsTable2Item[]>(paginationItems.slice(0, 10));
...
export const Draggable: Story = {
    render: () => ({
        components: { VsTable2 },
        setup() {
            const totalItems = ref<VsTable2Item[]>([]);
...
export const BoundItems: Story = {
    render: () => ({
        components: { VsTable2 },
        setup() {
            const pagedItems = ref<VsTable2Item[]>([]);
            const totalItems = ref<VsTable2Item[]>([]);
            const selectedItems = ref<VsTable2Item[]>([]);
            return {
