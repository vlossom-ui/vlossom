...
2. **vs-container 패딩 조정**: Drawer에 `layout-responsive`를 설정하면, Drawer가 열릴 때 `vs-container`의 해당 방향 padding이 자동으로 추가됩니다.
...
<template>
    <vs-layout>
        ...
        <!-- layout-responsive: Drawer 열림 → vs-container의 paddingLeft 자동 추가 -->
        <!-- Header absolute → paddingTop은 vs-container에 layout-responsive 없이도 자동 적용 -->
...
| `layoutResponsive` | `boolean`                                | `false`  | -        | `vs-layout` 안에서 Drawer 개폐 시 `vs-container` padding 자동 조정 여부 |
...
- **vs-layout 연동**: `vs-layout` 안에 배치하면 헤더/푸터 포지션에 따라 드로어 내부 padding 및 offset을 자동 조정. `layout-responsive`를 설정하면 드로어 개폐에 따라 `vs-container`의 padding도 자동으로 반응
