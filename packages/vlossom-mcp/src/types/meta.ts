...
/** Lightweight component summary used by facade search and coverage responses. */
export interface ComponentSummary {
    name: string;
    kebabName: string;
    description: string;
    availableVersion: string;
    /** props 개수 — 컴포넌트 복잡도 신호 */
    propsCount: number;
    /** v-model 지원 여부 (modelValue prop 또는 update:modelValue 이벤트 존재) */
    hasVModel: boolean;
    /** 참조하는 하위 StyleSet 이름 — 스타일 조합 힌트 */
