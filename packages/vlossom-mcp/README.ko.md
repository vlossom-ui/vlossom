...
| `get_component`               | 컴포넌트의 props, StyleSet, 이벤트, 슬롯을 반환합니다 _(예정)_                   |
| `search_components`           | 이름, 설명, props 전체를 대상으로 키워드 검색합니다 _(예정)_                     |
| `suggest_components`          | 자연어 사용 사례를 기반으로 컴포넌트를 추천합니다 _(예정)_                       |
| `compare_components`          | 두 컴포넌트를 나란히 비교하고 차이점과 추천을 제공합니다 _(예정)_                |
| `get_component_relationships` | 컴포넌트의 부모/자식/형제 관계를 반환합니다 _(예정)_                             |
| `get_component_source`        | 컴포넌트의 Vue 소스 파일을 그대로 반환합니다 _(예정)_                            |
| `get_directive`               | Vlossom 디렉티브(예: `v-vs-scroll-shadow`)의 사용법과 옵션을 반환합니다 _(예정)_ |
| `get_composables`             | `useColorScheme` 등 공개 컴포저블의 사용법을 반환합니다 _(예정)_                 |

### 스타일 시스템

| 도구                  | 설명                                                                            |
| --------------------- | ------------------------------------------------------------------------------- |
| `get_css_tokens`      | 모든 `--vs-*` CSS 변수의 기본값과 라이트/다크 변형을 반환합니다 _(예정)_        |
| `get_vlossom_options` | `createVlossom()` 플러그인의 사용 가능한 옵션을 예시와 함께 반환합니다 _(예정)_ |

### 코드 생성

| 도구                      | 설명                                                                              |
| ------------------------- | --------------------------------------------------------------------------------- |
| `generate_component_code` | 자연어 요구사항을 받아 Vue SFC 코드를 생성합니다 _(예정)_                         |
...
| `get_form_recipe`         | 로그인, 회원가입, 파일 업로드 등 완성된 폼 패턴 코드를 제공합니다 _(예정)_        |

### 진단 및 검증

| 도구                       | 설명                                                                   |
| -------------------------- | ---------------------------------------------------------------------- |
| `diagnose_issue`           | 증상 설명을 알려진 GitHub 이슈와 매칭하고 해결책을 제안합니다 _(예정)_ |
